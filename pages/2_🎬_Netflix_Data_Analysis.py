...
country_str = ','.join(movies_df['country'].tolist())
country_list = list(country_str.split(','))

cleaned_country_list = []

for i in country_list:
    i = i.strip()
    cleaned_country_list.append(i)

unique_country_list = []

for i in cleaned_country_list:
    if i not in unique_country_list:
        unique_country_list.append(i)


unique_country_list.remove("Unknown")
unique_country_list.remove("")


n_countries = len(unique_country_list)

# TODO: Ex 2.5: How many characters long are on average the title names?

movies_df['title_length'] = movies_df['title'].str.len()

avg_title_length = round(movies_df['title_length'].mean(),1)
...
selected_year = movies_df.loc[movies_df.release_year == year]
top_10_countries = selected_year['country'].value_counts().head(10)
...
movies_df_mov = movies_df.loc[movies_df.type == 'Movie']
movies_df_mov[['time', 'min']] = movies_df_mov['duration'].str.split(' ', expand=True)
movies_df_mov.drop(columns='min')

movies_df_mov['time'] = movies_df_mov['time'].astype("int64")

movies_avg_duration_per_year = round(movies_df_mov.groupby(['release_year'])['time'].mean(), 2)
