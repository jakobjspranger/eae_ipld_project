...
temps_df["AvgTemperatureCelsius"] = round(temps_df.AvgTemperatureFahrenheit.apply(lambda x: ((x-32)*(5/9))), 2)       # uncomment this line to complete it
...
min_temp_city = temps_df.loc[temps_df.AvgTemperatureCelsius == min_temp, "City"].values[0]  # TODO
min_temp_date = temps_df.loc[temps_df.AvgTemperatureCelsius == min_temp, "Date"].values[0]  # TODO

max_temp_city = temps_df.loc[temps_df.AvgTemperatureCelsius == max_temp, "City"].values[0]  # TODO
max_temp_date = temps_df.loc[temps_df.AvgTemperatureCelsius == max_temp, "Date"].values[0]  # TODO
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        ...
        city_df_period = city_df_period = city_df.loc[(city_df.Date >= start_date) & (city_df.Date <= end_date)] 
    ...
    plt.title(f"Avg. Temp. in {", ".join(selected_cities)} From {start_date} to {end_date}")   # TODO
    plt.xlabel("Date")  # TODO
    plt.ylabel("Avg. Temp. in C")  # TODO
    ...
    for city in selected_cities:
        ...
        # TODO plot each city line and use the label parameter to set the legend name for each city    
        plt.hist(city_df_period['AvgTemperatureCelsius'], bins=20, alpha = 0.6, label=city)    # TODO: use the city_df_period AvgTemperatureCelsius column as the data to plot, you can use the parameter bins=20

    plt.title(f"Histogram - Avg. Temp. in {", ".join(selected_cities)} From {start_date} to {end_date}")   # TODO
    plt.xlabel("Avg. Temperature in C")  # TODO
    plt.ylabel("Freqency")  # TODO
